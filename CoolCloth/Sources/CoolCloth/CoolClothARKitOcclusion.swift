...
/// Optional visionOS adapter that streams ARKit reconstruction meshes to CoolCloth.
public final class CoolClothARKitOcclusionProvider: @unchecked Sendable {
    private let session = ARKitSession()
    private let provider = SceneReconstructionProvider()
    private let lock = NSLock()
    private var meshesByID: [UUID: CoolClothOcclusionMesh] = [:]
    ...
    private func handle(_ update: AnchorUpdate<MeshAnchor>) {
        ...
        lock.withLock {
            ...
            case .added, .updated:
                ...
                meshesByID[anchor.id] = CoolClothOcclusionMesh(
            ...
            setCoolClothOcclusionMeshes(Array(meshesByID.values))
        }
    }
