...
final class CoolWaterRenderExtension: RenderExtension, @unchecked Sendable {
    ...
    private var lastSphereCenter = SIMD3<Float>(0, -1000, 0)
    ...
    private func encodeScene(_ context: RenderPassContext) {
        // Sphere ("ball") rendering is intentionally disabled for this HomeDesign build —
        // its resources are no longer fetched here and its draw call is removed below.
        ...
        encodeLock.withLock {
            ...
            initializeSceneGeometryIfNeeded(poolVertices: poolVertices)
            ...
            // Sphere ("ball") draw intentionally removed — HomeDesign has no use for it
            // and there's no upstream visibility toggle to gate it with instead.
    ...
    private func drawOcclusion(
        ...
        encoder.setCullMode(.none)
        // Pool and render-extension camera matrices are in scene-local space. ARKit
        // reconstruction anchors are in visual world space, so convert them back to
        // scene-local before using the engine's effective (scene-root-aware) VP matrix.
        let sceneRoot = SceneRootTransform.shared
        sceneRoot.updateIfNeeded()
        let visualWorldToScene = sceneRoot.inverseMatrix
        ...
        for mesh in meshes where mesh.indexCount > 0 && mesh.vertexStride > 0 {
            ...
            let meshToScene = visualWorldToScene * mesh.transform
            var mvp = context.camera.viewProjectionMatrix * meshToScene
            var meshToWorld = meshToScene
    ...
    private func initializeSceneGeometryIfNeeded(poolVertices: MTLBuffer) {
